...
class omega( chem_evol ):
    ...
    def __cal_m_frac_stars(self, i):
        ...
        self.sfrin = self.sfr_input[i-1] * self.history.timesteps[i-1]
...
###############################################################################################
    ...
    def plot_yield_input(self,fig=8,xaxis='mini',yaxis='C-12',iniZ=0.0001,masses=[],marker='o',color='r',shape='-',table='yield_tables/isotope_yield_table.txt',fsize=[10,4.5],fontsize=14,rspace=0.6,bspace=0.15,labelsize=15,legend_fontsize=14,):
	...
	if True:
		...
		for mini in masses:
			...
			if '[' in xaxis:
				...
				x.append( np.log10( yx1_frac/yx2_frac * x2_ini/x1_ini) )
				plt.xlabel(xaxis)
				headerx=xaxis
			elif 'mini' == xaxis:
				x.append(mini)
			...
			if '[' in yaxis:
				...
				y.append( np.log10( yy1_frac/yy2_frac * y2_ini/y1_ini) )
				plt.ylabel(yaxis)
				headery=yaxis
			#for yields
			else:
	...
	>>> plt.plot_spectro('[Fe/H]','[C/Fe]')
        ...
        #  Fornax  #
        ############
        if self.galaxy == 'fornax' and show_data:

            #[Mg/Fe]
            ########
            if xaxis=='[Fe/H]' and yaxis=='[Mg/Fe]':
                ...
                y_f10=[]
                y_f10_err=[]
                with open(global_path+"fornax_data/Kirby_et_al_2010_Fornax.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        if not obs_f10[2] == -30.0:
                            x_f10.append(obs_f10[0])
                            #x_f10_err.append(obs_f10[1])
                            y_f10.append(obs_f10[2])
                            #y_f10_err.append(obs_f10[3])
                f10.close()
                #plt_ps.errorbar(x_f10,y_f10,xerr=x_f10_err,yerr=y_f10_err,fmt=None,ecolor='c')
                plt_ps.plot(x_f10,y_f10,'x',color='c',label='Kirby et al. (2010)')

                #Define axis limits if not a sub-figure
                if not sub_plot:
                    plt.ylim(-1.4,1.4)
                    plt.xlim(-4.0,0.4)
                ...
                with open(global_path+"fornax_data/Kirby_et_al_2010_Fornax.txt", 'r') as f10:
                    for line in f10:
                        ...
                        if not obs_f10[2] == -30.0:
                            x_f10.append(obs_f10[0])
                            #x_f10_err.append(obs_f10[1])
                            y_f10.append(obs_f10[4])
                            #y_f10_err.append(obs_f10[5])
                f10.close()
                #plt_ps.errorbar(x_f10,y_f10,xerr=x_f10_err,yerr=y_f10_err,fmt=None,ecolor='c')
                plt_ps.plot(x_f10,y_f10,'x',color='c',label='Kirby et al. (2010)')
                ...
                with open(global_path+"fornax_data/Kirby_et_al_2010_Fornax.txt", 'r') as f10:
                    for line in f10:
                        ...
                        if not obs_f10[2] == -30.0:
                            x_f10.append(obs_f10[0])
                            #x_f10_err.append(obs_f10[1])
                            y_f10.append(obs_f10[6])
                            #y_f10_err.append(obs_f10[7])
                f10.close()
                #plt_ps.errorbar(x_f10,y_f10,xerr=x_f10_err,yerr=y_f10_err,fmt=None,ecolor='c')
                plt_ps.plot(x_f10,y_f10,'x',color='c',label='Kirby et al. (2010)')
                ...
                with open(global_path+"fornax_data/Kirby_et_al_2010_Fornax.txt", 'r') as f10:
                    for line in f10:
                        ...
                        if not obs_f10[2] == -30.0:
                            x_f10.append(obs_f10[0])
                            #x_f10_err.append(obs_f10[1])
                            y_f10.append(obs_f10[8])
                            #y_f10_err.append(obs_f10[9])
                f10.close()
                #plt_ps.errorbar(x_f10,y_f10,xerr=x_f10_err,yerr=y_f10_err,fmt=None,ecolor='c')
                plt_ps.plot(x_f10,y_f10,'x',color='c',label='Kirby et al. (2010)')
        ...
        #  Carina  #
        ############
        if self.galaxy == 'carina' and show_data:

            #[Na/Fe]
            ########
            if xaxis=='[Fe/H]' and yaxis=='[Na/Fe]':

                #Venn et al. (2012)
                x_f10=[]
                y_f10=[]
                with open(global_path+"carina_data/Venn_et_al_2012.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        x_f10.append(obs_f10[0])
                        y_f10.append(obs_f10[1])
                f10.close()

                plt_ps.plot(x_f10,y_f10,'o',color='r',label='Venn et al. (2012)')
            ...
            #[Chi/Fe]
            ########
            if xaxis=='[Fe/H]' and yaxis=='[Chi/Fe]':
        ...
        # Sculptor #
        ############
        if show_sculptor or (self.galaxy == 'sculptor' and show_data):

            #[O/Fe]
            ########
            if xaxis=='[Fe/H]' and yaxis=='[O/Fe]':

                #Hill et al. - Unpublished
                #x_f10=[]
                #y_f10=[]
                #with open(global_path+"sculptor_data/Hill_unpublished.txt", 'r') as f10:
                #    for line in f10:
                #        obs_f10 = [float(x) for x in line.split()]
                #        x_f10.append(obs_f10[0])
                #        y_f10.append(obs_f10[12])
                #f10.close()
                #plt_ps.plot(x_f10,y_f10,'o',color='r',label='Hill et al. - Unpublished')

                #Frebel (2010)
                x_f10=[]
                y_f10=[]
                with open(global_path+"sculptor_data/Frebel_2010_Sculptor.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        x_f10.append(obs_f10[0])
                        y_f10.append(obs_f10[3])
                f10.close()
                plt_ps.plot(x_f10,y_f10,'o',color='m',label='Frebel (2010) - compilation')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Na/Fe]':
                ...
                x_f10=[]
                y_f10=[]
                with open(global_path+"sculptor_data/Starkenburg_et_al_2013.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        x_f10.append(obs_f10[0])
                        y_f10.append(obs_f10[2])
                f10.close()
                plt_ps.plot(x_f10,y_f10,'^',color='g',label='Starkenburg_et_al_2013')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Mg/Fe]':
                ...
                y_f10=[]
                y_f10_err=[]
                with open(global_path+"sculptor_data/Kirby_et_al_2010.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        if not obs_f10[2] == -30.0:
                            x_f10.append(obs_f10[0])
        ...
        ##############################################
        #Add observed data if the galaxy is specified#
        ##############################################
        if (self.galaxy == 'milky_way' or self.galaxy == 'milky_way_cte') and show_data:

            #[C/Fe]
            ########
            if xaxis=='[Fe/H]' and yaxis=='[C/Fe]':

                #Frebel (2010)
                x_f10=[]
                y_f10=[]
                with open(global_path+"milky_way_data/Frebel_2010_Milky_Way.txt", 'r') as f10:
                    for line in f10:
                        obs_f10 = [float(x) for x in line.split()]
                        x_f10.append(obs_f10[0])
                        y_f10.append(obs_f10[1])
                f10.close()
                plt_ps.plot(x_f10,y_f10,'x',color='g',label='Frebel (2010) - compilation')

                #Spite et al. (2005)
                x_s05=[]
                y_s05=[]
                with open(global_path+"milky_way_data/Spite_et_al_2005.txt", 'r') as s05:
                    for line in s05:
                        obs_s05 = [float(x) for x in line.split()]
                        obs_s05[1]=np.log10( 10**(obs_s05[1]-3.48) / 10**(obs_s05[0]-4.50) ) - 1.02
                        x_s05.append(obs_s05[0])
                        y_s05.append(obs_s05[1])
                s05.close()
                plt_ps.plot(x_s05,y_s05,'o',color='b',label='Spite et al. (2005)')

                #Lai et al. (2008)
                x_l08=[]
                y_l08=[]
                with open(global_path+"milky_way_data/Lai_et_al_2008.txt", 'r') as l08:
                    for line in l08:
                        obs_l08 = [float(x) for x in line.split()]
                        x_l08.append(obs_l08[0])
                        y_l08.append(obs_l08[1])
                l08.close()
                plt_ps.plot(x_l08,y_l08,'o',color='r',label='Lai et al. (2008)')

                #Bonifacio et al. (2009)
                x_b09=[]
                y_b09=[]
                with open(global_path+"milky_way_data/Bonifacio_et_al_2009.txt", 'r') as b09:
                    for line in b09:
                        obs_b09 = [float(x) for x in line.split()]
                        x_b09.append(obs_b09[0])
                        y_b09.append(obs_b09[1])
                b09.close()
                plt_ps.plot(x_b09,y_b09,'^',color='m',label='Bonifacio et al. (2009)')

                #Fabbian et al. (2009)
                x_f09=[]
                y_f09=[]
                with open(global_path+"milky_way_data/Fabbian_et_al_2009.txt", 'r') as f09:
                    for line in f09:
                        obs_f09 = [float(x) for x in line.split()]
                        obs_f09[1]=np.log10( 10**(obs_f09[1]-3.61) / 10**(obs_f09[0]-4.55) ) - 1.02
                        x_f09.append(obs_f09[0])
                        y_f09.append(obs_f09[1])
                f09.close()
                plt_ps.plot(x_f09,y_f09,'^',color='c')
                with open(global_path+"milky_way_data/Fabbian_et_al_2009.txt", 'r') as f09:
                    for line in f09:
                        obs_f09 = [float(x) for x in line.split()]
                        obs_f09[2]=np.log10( 10**(obs_f09[2]-3.61) / 10**(obs_f09[0]-4.55) ) - 1.02
                        x_f09.append(obs_f09[0])
                        y_f09.append(obs_f09[2])
                f09.close()
                plt_ps.plot(x_f09,y_f09,'^',color='c',label='Fabian et al. (2009)')

                #Reddy et al. (2006)
                x_r06=[]
                y_r06=[]
                with open(global_path+"milky_way_data/Reddy_et_al_2006.txt", 'r') as r06:
                    for line in r06:
                        obs_r06 = [float(x) for x in line.split()]
                        obs_r06[1] += 0.04
                        x_r06.append(obs_r06[0])
                        y_r06.append(obs_r06[1])
                r06.close()
                plt_ps.plot(x_r06,y_r06,'^',color='r',label='Reddy et al. (2006)')

                #Reddy et al. (2003)
                x_r03=[]
                y_r03=[]
                with open(global_path+"milky_way_data/Reddy_et_al_2003.txt", 'r') as r03:
                    for line in r03:
                        obs_r03 = [float(x) for x in line.split()]
                        obs_r03[1] += 0.04
                        x_r03.append(obs_r03[0])
                        y_r03.append(obs_r03[1])
                r03.close()
                plt_ps.plot(x_r03,y_r03,'s',color='b',label='Reddy et al. (2003)')

                #Bensby & Feltzing (2006)
                x_bf06=[]
                y_bf06=[]
                with open(global_path+"milky_way_data/Bensby_Feltzing_2006.txt", 'r') as bf06:
                    for line in bf06:
                        obs_bf06 = [float(x) for x in line.split()]
                        obs_bf06[4]=np.log10( 10**(obs_bf06[4]-3.56) / 10**(obs_bf06[0]-4.50) ) - 1.02
                        x_bf06.append(obs_bf06[0])
                        y_bf06.append(obs_bf06[4])
                bf06.close()
                plt_ps.plot(x_bf06,y_bf06,'^',color='g',label='Bensby & Feltzing (2006)')

                #Cayrel et al. (2004)
                x_c04=[]
                y_c04=[]
                with open(global_path+"milky_way_data/Cayrel_et_al_2004.txt", 'r') as c04:
                    for line in c04:
                        obs_c04 = [float(x) for x in line.split()]
                        x_c04.append(obs_c04[0])
                        y_c04.append(obs_c04[1])
                c04.close()
                plt_ps.plot(x_c04,y_c04,'^',color='b',label='Cayrel et al. (2004)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[N/Fe]':
                ...
                with open(global_path+"milky_way_data/Spite_et_al_2005.txt", 'r') as s05:
                    for line in s05:
                        obs_s05 = [float(x) for x in line.split()]
                        obs_s05[2]=np.log10( 10**(obs_s05[2]-4.08) / 10**(obs_s05[0]-4.50) ) - 0.42
                        x_s05.append(obs_s05[0])
                        y_s05.append(obs_s05[2])
                s05.close()
                plt_ps.plot(x_s05,y_s05,'o',color='b',label='Spite et al. (2005)')
                ...
                x_i04=[]
                y_i04=[]
                with open(global_path+"milky_way_data/Israelian_et_al_2004.txt", 'r') as i04:
                    for line in i04:
                        obs_i04 = [float(x) for x in line.split()]
                        obs_i04[1]=np.log10( 10**(obs_i04[1]-3.95) / 10**(obs_i04[0]-4.50) ) - 0.42
                        x_i04.append(obs_i04[0])
                        y_i04.append(obs_i04[1])
                i04.close()
                plt_ps.plot(x_i04,y_i04,'^',color='c', label='Israelian et al. (2004)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[O/Fe]':
                ...
                with open(global_path+"milky_way_data/Reddy_et_al_2003.txt", 'r') as r03:
                    for line in r03:
                        obs_r03 = [float(x) for x in line.split()]
                        x_r03.append(obs_r03[0])
                        y_r03.append(obs_r03[3])
                r03.close()
                plt_ps.plot(x_r03,y_r03,'^',color='b')
                with open(global_path+"milky_way_data/Reddy_et_al_2003.txt", 'r') as r03:
                    for line in r03:
                        obs_r03 = [float(x) for x in line.split()]
                        x_r03.append(obs_r03[0])
                        y_r03.append(obs_r03[4])
                r03.close()
                plt_ps.plot(x_r03,y_r03,'s',color='b',label='Reddy et al. (2003)')
                ...
                x_g03=[]
                y_g03=[]
                with open(global_path+"milky_way_data/Gratton_et_al_2003.txt", 'r') as g03:
                    for line in g03:
                        obs_g03 = [float(x) for x in line.split()]
                        x_g03.append(obs_g03[0])
                        y_g03.append(obs_g03[2])
                g03.close()
                plt_ps.plot(x_g03,y_g03,'^',color='c')
                with open(global_path+"milky_way_data/Gratton_et_al_2003.txt", 'r') as g03:
                    for line in g03:
                        obs_g03 = [float(x) for x in line.split()]
                        x_g03.append(obs_g03[0])
                        y_g03.append(obs_g03[3])
                g03.close()
                plt_ps.plot(x_g03,y_g03,'s',color='c', label='Gratton et al. (2003)')
                ...
                with open(global_path+"milky_way_data/Fabbian_et_al_2009.txt", 'r') as f09:
                    for line in f09:
                        obs_f09 = [float(x) for x in line.split()]
                        obs_f09[4]=np.log10( 10**(obs_f09[4]-3.34) / 10**(obs_f09[0]-4.55) ) - 1.33
                        x_f09.append(obs_f09[0])
                        y_f09.append(obs_f09[4])
                f09.close()
                plt_ps.plot(x_f09,y_f09,'^',color='c',label='Fabian et al. (2009)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Na/Fe]':
                ...
                x_a07=[]
                y_a07=[]
                with open(global_path+"milky_way_data/Andrievsky_et_al_2007.txt", 'r') as a07:
                    for line in a07:
                        obs_a07 = [float(x) for x in line.split()]
                        x_a07.append(obs_a07[0])
                        y_a07.append(obs_a07[2])
                a07.close()
                plt_ps.plot(x_a07,y_a07,'o',color='g', label='Andrievsky et al. (2007)')
                ...
                x_g06=[]
                y_g06=[]
                with open(global_path+"milky_way_data/Gehren_et_al_2006.txt", 'r') as g06:
                    for line in g06:
                        obs_g06 = [float(x) for x in line.split()]
                        x_g06.append(obs_g06[0])
                        y_g06.append(obs_g06[2])
                g06.close()
                plt_ps.plot(x_g06,y_g06,'^',color='m', label='Gehren et al. (2006)')

                #Bensby et al. (2005)
                #x_b05=[]
                #y_b05=[]
                #with open(global_path+"milky_way_data/Bensby_et_al_2005.txt", 'r') as b05:
                #    for line in b05:
                #        obs_b05 = [float(x) for x in line.split()]
                #        obs_b05[5]=np.log10( 10**(obs_b05[5]-5.67) / 10**(obs_b05[3]-4.50) ) - 1.17
                #        x_b05.append(obs_b05[3])
                #        y_b05.append(obs_b05[5])
                #b05.close()
                #plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Mg/Fe]':
                ...
                x_a10=[]
                y_a10=[]
                with open(global_path+"milky_way_data/Andrievsky_et_al_2010.txt", 'r') as a10:
                    for line in a10:
                        obs_a10 = [float(x) for x in line.split()]
                        x_a10.append(obs_a10[0])
                        y_a10.append(obs_a10[2])
                a10.close()
                plt_ps.plot(x_a10,y_a10,'o',color='g', label='Andrievsky et al. (2010)')
                ...
                with open(global_path+"milky_way_data/Bensby_et_al_2005.txt", 'r') as b05:
                    for line in b05:
                        obs_b05 = [float(x) for x in line.split()]
                        obs_b05[6]=np.log10( 10**(obs_b05[6]-4.42) / 10**(obs_b05[3]-4.50) ) - 0.08
                        x_b05.append(obs_b05[3])
                        y_b05.append(obs_b05[6])
                b05.close()
                plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Al/Fe]':
                ...
                #    for line in b05:
                #        obs_b05 = [float(x) for x in line.split()]
                #        obs_b05[7]=np.log10( 10**(obs_b05[7]-5.53) / 10**(obs_b05[3]-4.50) ) - 1.03
                #        x_b05.append(obs_b05[3])
                #        y_b05.append(obs_b05[7])
                #b05.close()
                #plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
                ...
                x_a08=[]
                y_a08=[]
                with open(global_path+"milky_way_data/Andrievsky_et_al_2008.txt", 'r') as a08:
                    for line in a08:
                        obs_a08 = [float(x) for x in line.split()]
                        x_a08.append(obs_a08[0])
                        y_a08.append(obs_a08[2])
                a08.close()
                plt_ps.plot(x_a08,y_a08,'o',color='g', label='Andrievsky et al. (2008)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Si/Fe]':
                ...
                with open(global_path+"milky_way_data/Bensby_et_al_2005.txt", 'r') as b05:
                    for line in b05:
                        obs_b05 = [float(x) for x in line.split()]
                        obs_b05[8]=np.log10( 10**(obs_b05[8]-4.45) / 10**(obs_b05[3]-4.50) ) - 0.05
                        x_b05.append(obs_b05[3])
                        y_b05.append(obs_b05[8])
                b05.close()
                plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[S/Fe]':
                ...
                x_c05=[]
                y_c05=[]
                with open(global_path+"milky_way_data/Caffau_et_al_2005.txt", 'r') as c05:
                    for line in c05:
                        obs_c05 = [float(x) for x in line.split()]
                        x_c05.append(obs_c05[0])
                        y_c05.append(obs_c05[1])
                c05.close()
                plt_ps.plot(x_c05,y_c05,'o',color='r', label='Caffau et al. (2005)')

                #Nissen et al. (2007)
                x_n07=[]
                y_n07=[]
                with open(global_path+"milky_way_data/Nissen_et_al_2007.txt", 'r') as n07:
                    for line in n07:
                        obs_n07 = [float(x) for x in line.split()]
                        x_n07.append(obs_n07[0])
                        y_n07.append(obs_n07[1])
                n07.close()
                plt_ps.plot(x_n07,y_n07,'^',color='g')
                with open(global_path+"milky_way_data/Nissen_et_al_2007.txt", 'r') as n07:
                    for line in n07:
                        obs_n07 = [float(x) for x in line.split()]
                        x_n07.append(obs_n07[0])
                        y_n07.append(obs_n07[2])
                n07.close()
                plt_ps.plot(x_n07,y_n07,'^',color='g', label='Nissen et al. (2007)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Ca/Fe]':
                ...
                with open(global_path+"milky_way_data/Bensby_et_al_2005.txt", 'r') as b05:
                    for line in b05:
                        obs_b05 = [float(x) for x in line.split()]
                        obs_b05[9]=np.log10( 10**(obs_b05[9]-5.64) / 10**(obs_b05[3]-4.50) ) + 1.14
                        x_b05.append(obs_b05[3])
                        y_b05.append(obs_b05[9])
                b05.close()
                plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Ti/Fe]':
                ...
                with open(global_path+"milky_way_data/Cayrel_et_al_2004.txt", 'r') as c04:
                    for line in c04:
                        obs_c04 = [float(x) for x in line.split()]
                        x_c04.append(obs_c04[0])
                        y_c04.append(obs_c04[11])
                        y_c04_2.append(obs_c04[12])
                c04.close()
                plt_ps.plot(x_c04,y_c04_2,'^',color='b')
                plt_ps.plot(x_c04,y_c04,'^',color='b',label='Cayrel et al. (2004)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Cr/Fe]':
                ...
                x_r06=[]
                y_r06=[]
                with open(global_path+"milky_way_data/Reddy_et_al_2006_2.txt", 'r') as r06:
                    for line in r06:
                        obs_r06 = [float(x) for x in line.split()]
                        x_r06.append(obs_r06[0])
                        y_r06.append(obs_r06[1])
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Mn/Fe]':
                ...
                with open(global_path+"milky_way_data/Gratton_et_al_2003.txt", 'r') as g03:
                    for line in g03:
                        obs_g03 = [float(x) for x in line.split()]
                        x_g03.append(obs_g03[0])
                        y_g03.append(obs_g03[15])
                g03.close()
                plt_ps.plot(x_g03,y_g03,'s',color='c', label='Gratton et al. (2003)')

                #Bergemann_Gehren (2008)
                x_bg08=[]
                y_bg08=[]
                with open(global_path+"milky_way_data/Bergemann_Gehren_2008.txt", 'r') as bg08:
                    for line in bg08:
                        obs_bg08 = [float(x) for x in line.split()]
                        x_bg08.append(obs_bg08[0])
                        y_bg08.append(obs_bg08[1])
                bg08.close()
                plt_ps.plot(x_bg08,y_bg08,'o',color='g', label='Bergemann & Gehren (2008)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Cu/Fe]':
                ...
                x_b04=[]
                y_b04=[]
                with open(global_path+"milky_way_data/Bihain_et_al_2004.txt", 'r') as b04:
                    for line in b04:
                        obs_b04 = [float(x) for x in line.split()]
                        obs_b04[1]=np.log10( 10**(obs_b04[1]-7.79) / 10**(obs_b04[0]-4.50) ) + 3.29
                        x_b04.append(obs_b04[0])
                        y_b04.append(obs_b04[1])
                b04.close()
                plt_ps.plot(x_b04,y_b04,'o',color='g', label='Bihain et al. (2004)')
            ...
            if xaxis=='[Fe/H]' and yaxis=='[Zn/Fe]':
                ...
                with open(global_path+"milky_way_data/Bihain_et_al_2004.txt", 'r') as b04:
                    for line in b04:
                        obs_b04 = [float(x) for x in line.split()]
                        obs_b04[2]=np.log10( 10**(obs_b04[2]-7.40) / 10**(obs_b04[0]-4.50) ) + 2.90
                        x_b04.append(obs_b04[0])
                        y_b04.append(obs_b04[2])
                b04.close()
                plt_ps.plot(x_b04,y_b04,'o',color='g', label='Bihain et al. (2004)')
                ...
                with open(global_path+"milky_way_data/Bensby_et_al_2005.txt", 'r') as b05:
                    for line in b05:
                        obs_b05 = [float(x) for x in line.split()]
                        obs_b05[15]=np.log10( 10**(obs_b05[15]-7.40) / 10**(obs_b05[3]-4.50) ) + 2.90
                        x_b05.append(obs_b05[3])
                        y_b05.append(obs_b05[15])
                b05.close()
                plt_ps.plot(x_b05,y_b05,'o',color='m', label='Bensby et al. (2005)')
            ...
            #[C/O] vs [O/H]
            ###############
            if xaxis=='[O/H]' and yaxis=='[C/O]':

                #Akerman et al. (2004)
                x_a04=[]
                y_a04=[]
                with open(global_path+"milky_way_data/Akerman_et_al_2004.txt", 'r') as a04:
                    for line in a04:
                        obs_a04 = [float(x) for x in line.split()]
                        x_a04.append(obs_a04[2])
                        y_a04.append(obs_a04[1])
                a04.close()
                plt_ps.plot(x_a04,y_a04,'s',color='r', label='Akerman et al. (2004)')
                ...
                with open(global_path+"milky_way_data/Fabbian_et_al_2009.txt", 'r') as f09:
                    for line in f09:
                        obs_f09 = [float(x) for x in line.split()]
                        x_f09.append(obs_f09[4])
                        y_f09.append(obs_f09[6])
                f09.close()
                plt_ps.plot(x_f09,y_f09,'^',color='c',label='Fabian et al. (2009)')
                ...
                with open(global_path+"milky_way_data/Spite_et_al_2005.txt", 'r') as s05:
                    for line in s05:
                        obs_s05 = [float(x) for x in line.split()]
                        obs_s05[1]=np.log10( 10**(obs_s05[1]-3.48) / 10**(obs_s05[3]-3.26) ) + 0.31
                        obs_s05[3]=obs_s05[3] - 3.26 + 3.17
                        x_s05.append(obs_s05[3])
                        y_s05.append(obs_s05[1])
                s05.close()
                plt_ps.plot(x_s05,y_s05,'o',color='b',label='Spite et al. (2005)')
                ...
                with open(global_path+"milky_way_data/Bensby_Feltzing_2006.txt", 'r') as bf06:
                    for line in bf06:
                        obs_bf06 = [float(x) for x in line.split()]
                        x_bf06.append(obs_bf06[2])
                        y_bf06.append(obs_bf06[3])
                bf06.close()
                plt_ps.plot(x_bf06,y_bf06,'^',color='g',label='Bensby & Feltzing (2006)')
...
#		print bin_values
    ...
    def write_evol_table(self,elements=['H'],isotopes=['H-1'],table_name='gce_table.txt',interact=False):
	...
	if notebookmode==True:
		...
		#return display.FileLink('../../nugrid/SYGMA/SYGMA_online/SYGMA_dev/evol_table/'+name)
                if interact==False:
                        return HTML("""<a href="""+"""evol_tables/"""+name+""">Download</a>""")
                else:
                        return name
	else:
		print 'file '+table_name+' saved in subdirectory evol_tables.'
		f1=open(global_path+'evol_tables/'+table_name,'w')
        	f1.write(out)
        	f1.close()
